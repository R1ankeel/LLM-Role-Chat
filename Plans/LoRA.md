...
### Спринт 5 — Frontend (Vue) — **✅ ВЫПОЛНЕН (2026-08-07)**

**Цель:** вкладка «LoRA» в модальном окне настроек и индикация в шапке чата.

**Статус:** выполнен. Все задачи 1–8 закрыты; проверка — `npm run build` (vue-tsc) без ошибок,
мануальные сценарии (фронт-тест-раннер отсутствует). Итоговая реализация:

- **Вкладка (задачи 1–2):** `SettingsTab` расширен на `'lora'` (`frontend/src/stores/ui.ts`);
  вкладка «LoRA» добавлена в `SettingsTabs.vue` (обязательная, ТЗ §3) и подключена в
  `SettingsModal.vue` после «Локации».
- **Новый `frontend/src/components/settings/LoRASettings.vue`** — **две логические части (§2.6)**:
  - **«Доступные LoRA» (глобальный registry):** список зарегистрированных адаптеров (name, путь,
    base model/identity, статус Compatible/Incompatible/Unknown относительно базовой модели текущего
    чата), кнопка «+ Добавить LoRA» → форма (название/путь/формат/base model identity/базовая модель
    для справки/описание), редактирование, удаление (409 → список использующих чатов).
  - **«LoRA этого чата» (конфигурация):** тумблер «Включить LoRA» + селектор ровно одного адаптера
    из registry + кнопка «Убрать».
- **Три состояния `lora_enabled` (§2.4):** `enabled=true` без адаптера → предупреждение «LoRA
  включена, но адаптер не выбран»; `enabled=false` → нейтральный вид; `enabled=true` + адаптер →
  рабочий вид.
- **Ограничения runtime в UI (§2.5):** ровно один адаптер (select, мультивыбор и weight-контролы
  отсутствуют); `supports_safetensors=false` — в форме регистрации только GGUF/auto (safetensors
  недоступен), с подсказкой; статус `Unknown` — явное предупреждение (badge в списке + inline-блок
  при выборе адаптера), не блокирует молча.
- **API-слой (задача 6):** `frontend/src/api/lora.ts`
  (`fetch/create/update/deleteLoraAdapter`, `fetch/updateChatLoraConfig`) + типы (`types/lora.ts`:
  `LoRAAdapter`, `ChatLoRAConfig`, `CompatibilityStatus`; `api/types.ts` — методы интерфейса `Api`) +
  mocks (`mocks/data.ts`: `mockLoraAdapters`, `mockChatLoraConfig`, `base_model_identity` у
  mock-чатов; `mocks/service.ts`) + Pinia-стор `stores/lora.ts` с **раздельными** состояниями/
  действиями: registry (`adapters`, `load/create/update/deleteAdapter`) и chat config (`config`,
  `loadConfig/saveConfig`).
- **`ChatHeader.vue` (задача 7):** бейдж-индикатор «LoRA» при активной конфигурации (`enabled=true`
  + выбранный адаптер, §2.4); конфигурация грузится через `lora.loadConfig(currentChatId)`.
- **Единый источник состояния (задача 8):** единственный объект `{enabled, adapter_id}`; в
  компоненте — локальный draft, синхронизируемый ТОЛЬКО из серверного ответа; после Save источник
  истины — `lora.config` (сервер); без сабмита несохранённые изменения сбрасываются (компонент
  пересоздаётся при закрытии модалки / смене чата).
- **Backend-уточнение (небольшое, Sprint 5):** в `app/schemas.py` `ChatRead` добавлено
  `base_model_identity: Optional[str] = None` — UI получает identity базовой модели чата для статуса
  совместимости §2.3 (без него статус всегда `Unknown`). Обратная совместимость: у старого backend
  поле отсутствует → статус `Unknown` (честное предупреждение, не silent fallback).

**Критерий готовности:** ✅ полный цикл «включить → выбрать адаптер → сохранить → перезагрузить →
состояние восстановлено» (GET/PUT `/api/chats/{id}/lora` — источник истины); ✅ без сабмита
несохранённые изменения сбрасываются; ✅ registry и конфигурация чата не смешиваются в UI (две
секции, раздельные состояния стора); ✅ ограничения runtime показаны явно.

**Проверка:** `npm run build` (vue-tsc) без ошибок; мануальные сценарии в mock-режиме
(`VITE_USE_MOCKS=true`) и против реального backend (фронт-тест-раннер отсутствует). Backend после
уточнения `ChatRead` — полный LoRA-прогон 76 passed + chat/memory/ollama 41 passed.
...
Покрытие 32–39 — **мануальные сценарии** (тумблер → селектор → «Убрать», три состояния §2.4,
форма регистрации с GGUF-only, индикатор в шапке, восстановление после перезагрузки) в mock-режиме
и против реального backend; фронт-тест-раннер отсутствует, `npm run build` (vue-tsc) без ошибок.
