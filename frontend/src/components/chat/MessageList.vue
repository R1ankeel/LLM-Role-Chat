...
const streamingTempId = computed(() => messages.streaming?.tempId ?? null)
  ...
  <div class="message-list" ref="scrollEl" @scroll.passive="onScroll">
    <div class="message-list__inner">
      <template v-if="hasContent">
        <div v-for="item in feed" :key="itemKey(item)" class="message-list__row">
          ...
          <MessageItem
            ...
            :is-streaming="item.message.id === streamingTempId"
          />
      ...
      <div class="message-list__indicator">
        <GenerationIndicator :status="messages.status" :name="messages.generatingName" />
      </div>
    </div>
