...
# ---------------------------------------------------------------------------
# LoRA runtime (Plans/LoRA.md, Sprint 2): создание/загрузка/проверка runtime-
# моделей через HTTP API Ollama, БЕЗ shell/subprocess (ТЗ §35, §2.7).
#
# Пайплайн создания runtime-модели (§2.7): sha256 файла адаптера →
# HEAD /api/blobs/:digest (200/404) → при 404 POST байтами файла →
# POST /api/create {from, adapters}. Modelfile-строку НЕ передаём (Ollama
# 0.32.6 возвращает 400, Sprint 0); в `adapters` ровно один адаптер (§2.5).
# Ошибки → RuntimeError с текстом (не silent fallback, §7).
# ---------------------------------------------------------------------------
