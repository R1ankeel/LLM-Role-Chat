...
# Глобальный registry (`lora_adapters`) + конфигурация чата (`chat_lora_adapters`
# + `Chat.lora_enabled`). MVP (§2.5): ровно один адаптер на чат (UNIQUE(chat_id)),
# без weight/order_index. Валидация пути — `lora_validation` (§2.7): абсолютный,
# существует, читаемый, валидный GGUF; при create/update вычисляется sha256.
# Физический файл адаптера НИКОГДА не удаляется — только регистрация (§2.7).
