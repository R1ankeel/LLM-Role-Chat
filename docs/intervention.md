...
- Инструкция хранится **только в памяти** (`app/pending_intervention.py`) — она
  никогда не попадает в таблицу `messages` и не переживает рестарт процесса.
- После **полностью успешного** раунда перед потреблением инструкции в память
  каждого затронутого персонажа записывается **незаметный факт** вида
  «Игрок попросил: …» (категория `событие`, `memory_type=episodic`,
  `importance=0.9`, без `source_message_ids`, чтобы факт не был отфильтрован
  witness-фильтром для персонажей, не воспринявших реплики раунда). Слово
  «вмешательство» в факте не используется — он читается как обычное событие
  истории, чтобы затронутые боты не «теряли» контекст на следующих ходах.
