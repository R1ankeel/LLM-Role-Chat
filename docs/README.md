...
| [lora.md](lora.md) | LoRA-адаптеры (MVP: одна LoRA на чат, без весов): модель данных, миграции, валидация пути (§2.7), CRUD; план — `Plans/LoRA.md` |
...
- **Ключевые системы**:
  ...
  - *LoRA (Sprint 1)* — регистр LoRA-адаптеров (`lora_adapters`) + конфигурация чата (одна LoRA на чат, `lora_enabled`); валидация пути GGUF, sha256 файла. Runtime-интеграция в основной `generate()` — в спринтах 2–3 (см. `lora.md`).
