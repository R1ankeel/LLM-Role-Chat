...
> Код-база: `ai-roleplay-chat/app/{models,schemas,config,database,relationship_service,relationship_analyzer,chat_engine,context_builder,prompt_builder,ollama_client,crud}.py`, `routers/relationships.py`, `tests/test_relationship_*.py`.
...
- **Валидация PUT** (`routers/relationships.py:76`): `relationship_type` по
  `relationship_valid_types` + графу.
- **Batch commits:** один flush/commit на раунд в цикле пар.
...
relationship_service                   (apply_delta + issue create/resolve)  ✅
   ↓
RelationshipEvent (kind, snapshot_after, source_message_ids, round_id) + state update   (❌ Sprint 3)
   ↓
...
- `RelationshipEvent` += `kind` (`llm|decay|manual`), `affection_after … jealousy_after`,
  `source_message_ids` (Text JSON), `round_id` (Text). ❌ (Sprint 3)
...
- `CharacterRelationshipUpdate` — валидация `relationship_type`. ❌ (Sprint 4)
...
RELATIONSHIP_DECAY_JEALOUSY_PER_ROUND   = 3
RELATIONSHIP_DECAY_RESENTMENT_PER_ROUND = 1
...
RELATIONSHIP_EVENTS_MAX_PER_PAIR    = 100
```
